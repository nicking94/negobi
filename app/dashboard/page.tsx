      ...
      <div className="flex flex-col flex-1 w-full">
        ...
        <main className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-gray_xxl to-gray_l/20 overflow-y-auto">
          <div className="mb-6 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
            <div className="flex gap-2 flex-wrap">
              <button
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  timeRange === "day"
                    ? "bg-green_b text-white"
                    : "bg-white text-gray_b"
                }`}
            ...
            <select
              className="px-4 py-2 rounded-lg border border-gray_xl bg-white text-gray_b text-sm"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 lg:mb-8">
              ...
              <CardContent>
                ...
                <div
                  className={`flex items-center text-sm ${
                    metrics.revenue.comparison >= 0
                      ? "text-green_m"
                      : "text-red_m"
                  }`}
                >
                  {metrics.revenue.comparison >= 0 ? "↑" : "↓"}{" "}
                  {Math.abs(metrics.revenue.comparison)}% vs período anterior
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
              ...
              <CardContent>
                ...
                <div
                  className={`flex items-center text-sm ${
                    metrics.orders.comparison >= 0
                      ? "text-green_m"
                      : "text-red_m"
                  }`}
                >
                  {metrics.orders.comparison >= 0 ? "↑" : "↓"}{" "}
                  {Math.abs(metrics.orders.comparison)}% vs período anterior
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
              ...
              <CardContent>
                ...
                <div
                  className={`flex items-center text-sm ${
                    metrics.customers.comparison >= 0
                      ? "text-green_m"
                      : "text-red_m"
                  }`}
                >
                  {metrics.customers.comparison >= 0 ? "↑" : "↓"}{" "}
                  {Math.abs(metrics.customers.comparison)}% vs período anterior
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatCurrency(metrics.avgOrder.total)}
                </div>
                <div
                  className={`flex items-center text-sm ${
                    metrics.avgOrder.comparison >= 0
                      ? "text-green_m"
                      : "text-red_m"
                  }`}
                >
                  {metrics.avgOrder.comparison >= 0 ? "↑" : "↓"}{" "}
                  {Math.abs(metrics.avgOrder.comparison)}% vs período anterior
                </div>
              </CardContent>
            </Card>
          </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {topProducts.map((product, index) => (
                    <div
                      key={product.id}
                      className="flex items-center justify-between p-2 hover:bg-gray_xxl rounded-lg"
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-medium text-gray_m">
                          {index + 1}
                        </span>
                        <div className="w-10 h-10 bg-gray_xl rounded-md flex items-center justify-center">
                          {product.image ? (
                            <Image
                              src={product.image}
                              alt={product.name}
